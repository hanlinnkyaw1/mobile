...
// Only imports files that actually exist on disk.
// ============================================================
...
// ── N3 ──
// N3MojiGoiTest1 is missing — file is named N3mojigoiTest1.json (lowercase m)
...
// ── N5 ──
// N5 Reading 2/3 and Listening 2/3 and MojiGoi 2/3 are missing from disk
...
/**
 * Return the list of available (level, testNum) pairs.
 * Used by TestSelection to only show tests that have data.
 */
export function getAvailableTests() {
  const available = [];
  for (const [level, tests] of Object.entries(REGISTRY)) {
    for (const testNum of Object.keys(tests)) {
      available.push({ level, testNum: Number(testNum) });
    }
  }
  return available;
}
